...
Five things that will otherwise cost you an evening:
...
* **A hot-path read may not grow with the run.** The world never ends, so a
  fold over an append-only table is a slow leak: the spend ledger's fold ran
  four times per model call and by day three was 80% of the production
  database's time on an eighth of a vCPU (LLM-0010). Per tick, per call and
  per poll, read one row, an index range, or a running total the journal can
  rebuild. `py/tests/test_query_plans.py` holds the hot queries to it — add
  yours there, and EXPLAIN it against production first (`docs/deployment.md`,
  "Watching load").
