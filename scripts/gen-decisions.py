...
def record_paths() -> list[Path]:
    ...
    Two patterns, because they cover different shapes: cross-cutting records
    nest one level under `decisions/` (`decisions/core/CORE-0001-x.md`), while
    area records sit directly in a package's own `decisions/` folder.
    """
...
def check_records(records: list[Record]) -> list[str]:
    ...
    for record in records:
        ...
        # CORE lives at decisions/core/; every other prefix lives beside its code.
        ...
        elif folder.is_relative_to(DECISIONS):
            problems.append(
                f"{source}: {record.prefix} is scoped to an area, so the record "
                f"belongs next to that code, not under decisions/"
            )
...
def render_rule(record: Record) -> str:
    """A Cursor project rule.

    `globs` is a comma-separated string, which is what Cursor expects even
    though it is not valid YAML — which is also why these files are kept away
    from YAML tooling. The description is the whole retrieval surface: it has
    to contain the words someone would use when the rule should fire.
    """

    tags = ", ".join(record.meta["tags"])
    description = (
        f"{record.id}: {record.meta['title']}. {record.summary} "
        f"Consult before changing this area, or when asking why it works this "
        f"way, what was decided, or whether it can be changed. Topics: {tags}."
    )
    return (
        "---\n"
        f"description: {description}\n"
        f"globs: {','.join(record.meta['scope'])}\n"
        "alwaysApply: false\n"
        "---\n\n"
        f"<!-- GENERATED by scripts/gen-decisions.py from {record.rel}. -->\n\n"
        f"# {record.id} — {record.meta['title']}\n\n"
        f"Status: {record.meta['status']} ({record.meta['date']}). "
        f"The record is canonical: {record.rel}\n\n"
        f"{record.summary}\n\n"
...
def rule_targets(records: list[Record]) -> dict[Path, str]:
    """Only accepted records with a real scope steer an agent.

    A superseded or proposed decision that still had a rule file would keep
    ...
    for record in records:
        if record.meta["status"] != "accepted" or not record.meta["scope"]:
            continue
        targets[CURSOR_RULES / f"decision-{record.id.lower()}.mdc"] = render_rule(
            record
        )
    return targets
...
def generate(records: list[Record], *, write: bool) -> list[str]:
    ...
    stale = sorted(
        path
        for path in CURSOR_RULES.glob("decision-*.mdc")
        if path not in wanted
    )
    changed = [p.relative_to(ROOT).as_posix() for p in stale]
...
def scaffold(records: list[Record], prefix: str, title: str) -> Path:
    ...
    folder = DECISIONS / "core" if prefix == "CORE" else DECISIONS / "core"
    ...
    meta.update(
        {"id": new_id, "title": title, "date": date.today().isoformat()}
    )
    path.parent.mkdir(parents=True, exist_ok=True)
