...
export function Dashboard({
  ...
  const select = useCallback(async (seq: number) => {
    if (selected === seq) {
      setSelected(null);
      setChain(null);
      return;
    ...
    try {
      ...
      for (const event of up.events) depths.set(event.seq, -(event.depth ?? 0));
      for (const event of down.events) depths.set(event.seq, event.depth ?? 0);
      setChain(depths);
    } catch {
      setChain(null);
    }
              ...
              <span>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 2,
                    background: ORG_COLORS[o.id] ?? "#888",
                    marginRight: 8,
                  }}
                />
                {o.name}
