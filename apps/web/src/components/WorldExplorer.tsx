  ...
  return (
    <main className="world-page">
      <header className="strip">
        ...
        <span className="muted">drag to pan · scroll to zoom · click anyone</span>
        <Link href="/" className="muted" style={{ marginLeft: "auto" }}>
          ← timeline
        </Link>
      </header>
      <div className="world-body">
        <div ref={container} className="world-canvas" data-testid="world-view" />
        <aside className="world-side">
          {agentId !== null ? (
            ...
            <div className="panel" data-testid="world-hint">
              ...
              <p className="muted">
                Click a person to see what they last decided and the distribution
                Jev returned for it. Click a building for the firm&apos;s books.
              </p>
              <p className="muted">
                Grey figures are customers: real demand, drawn as a sampled crowd.
                They are flows, not people with positions.
              </p>
            </div>
          )}
        </aside>
...
function Bars({ name, dist, draw }: { name: string; dist: Record<string, number>; draw?: number }) {
      ...
      {entries.map(([option, p]) => (
        <div className="dist-row" key={option} data-testid="distribution-bar">
          ...
          <span className="dist-track">
            <span className="dist-fill" style={{ width: `${Math.round(p * 100)}%` }} />
          </span>
...
function AgentPanel({ id, tick }: { id: string; tick: number }) {
  ...
  if (error !== null) return <div className="panel bad">Could not load {id}: {error}</div>;
  ...
  return (
    <div className="panel" data-testid="agent-panel" data-person={data.id}>
      ...
      {d === null ? (
        ...
        <>
          <p>
            <span className="pill" data-testid="decided-by">{d.source}</span>{" "}
            <code>{d.question_set}</code> <span className="muted">{d.label}</span>
          </p>
      ...
      {data.last_encounter === null ? (
        ...
        <dl className="fields" data-testid="last-encounter">
          ...
          <dt>started by</dt>
          <dd>{data.last_encounter.initiated ? data.name : data.last_encounter.with_name}</dd>
          <dt>led to</dt>
          <dd>
...
function Imagined({ seq }: { seq: number }) {
  ...
  useEffect(() => {
    let live = true;
    fetch(`${API}/encounters/${seq}/dialogue?generate=false`, { cache: "no-store" })
      .then((r) => r.json())
      .then((raw) => {
  ...
  return (
    <div className="prose" data-testid="prose">
      ...
      <p className="muted prose-note">
        ...
        {state.prose.cached ? " (cached)" : ` for $${state.prose.cost_usd.toFixed(5)}`} ·
  ...
  return (
    <div className="panel" data-testid="org-panel" data-org={data.id}>
      ...
      <dl className="fields">
        ...
        <dt>going on</dt>
        <dd>{data.active.length > 0 ? data.active.join("; ") : "nothing unusual"}</dd>
      </dl>
    </div>
  );
}
